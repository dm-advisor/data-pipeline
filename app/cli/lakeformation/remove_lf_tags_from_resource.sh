#!/bin/bash
##################################################################################################
# This script can be used to remove Lake Formation tags from tables and columns for a project.
#
# While coding/editing the CSV input file be sure to use Linux line feed (LF) instead of
# Windows (CRLF).
#
# To verify that tags have been removed properly, run the following script from
...
if [ "$#" -ne 1 ]; then
  ...
  printf "\nExample: %s ../../config/lakeformation/tags/remove_lf_tags_from_resource.csv" "$SCRIPT_NAME"
  exit 1
fi
...
json_table_template="../../config/lakeformation/tags/lf_tags_table_template.json"
json_columns_template="../../config/lakeformation/tags/lf_tags_columns_template.json"
...
do
    ...
    info "Now removing tags from table: ${resource_name} in database: ${db_name},"
    ...
    info "Now removing tags from columns: ${column_names} in table: ${resource_name} in database: ${db_name},"
    info "using LF tag key: ${lf_tag_key} and tag value: ${lf_tag_value}"
    # Substitute the variables in the JSON template with actual values from CSV file.
    temp_v=$(sed -e "s/\${DB_NAME}/${db_name}/" \
                 -e "s/\${TABLE_NAME}/${resource_name}/" \
  ...
  aws lakeformation remove-lf-tags-from-resource --cli-input-json \
    "${temp_v}" \
    || error "aws lakeformation remove-lf-tags-from-resource failed!"
