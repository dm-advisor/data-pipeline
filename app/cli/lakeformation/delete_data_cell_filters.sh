#!/bin/bash
##################################################################################################
# This script can be used to delete Lake Formation data cell filters for a project.
#
...
if [ "$#" -ne 1 ]; then
  ...
  printf "\nExample: %s ../../config/lakeformation/data-cell-filters/delete_data_cell_filters.csv" "$SCRIPT_NAME"
  exit 1
fi
...
json_template="../../config/lakeformation/data-cell-filters/delete_data_cell_filter_template.json"
...
while IFS='|' read -r filter_name db_name table_name
do
  info "Now deleting the data cells filter: ${filter_name} for table: ${table_name} in database: ${db_name}"
  ...
  aws lakeformation delete-data-cells-filter --cli-input-json \
    "${temp_v}" \
    || error "aws lakeformation delete-data-cells-filter failed!"
